...
const reversedString = (str) => {
  return str.split('').reduce((accumulator, letter, idx) => {
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((accumulator, person, idx) => {
    if(person.children) accumulator += person.children.length;
    return accumulator;
  },0);
};
...
const calculateAverage = (arr) => {
  let totals = arr.reduce((accumulator, val, idx) => {
    if(accumulator.count) accumulator.count++;
    else accumulator.count = 1;
    if(accumulator.sum) accumulator.sum += val;
    else accumulator.sum = val;
    return accumulator;
  },{});
...
const extractStat = (statName, arr) => {
  return arr.reduce((accumulator, stat, idx) => {
    if(stat.stat.name === statName) accumulator = stat;
    return accumulator;
  });
};
...
const extractChildren = (arr) => {
  const filteredArr = arr.filter(character => {
    return character.name.includes('a');
  })
  return (filteredArr.reduce((accumulator, character, idx) => {
    if(character.children) accumulator = `${accumulator},${character.children.join()}`;
    return accumulator;
  })).split(',');
};
