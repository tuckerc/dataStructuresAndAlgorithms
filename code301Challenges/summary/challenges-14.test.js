...
let biggerThanLuke = (arr) => {
  // define Luek's mass
  const luke = arr.find(character => {
    return character.name === 'Luke Skywalker';
  });
  // reduce the array to characters with mass > Luke's mass
  return arr.reduce((returnStr, character, idx, arr) => {
    if(Number(character.mass) > Number(luke.mass)) {
      if(idx === 0) {
        returnStr = `${character.name} - `;
      }
      else if(idx === arr.length - 1) {
        returnStr += character.name;
      }
      else returnStr += `${character.name} - `;
    }
    return returnStr;
  },'');
};
...
const sortBy = (property, arr) => {
  arr.sort((a, b) => {
    if((typeof a[property]) === 'number') return a[property] - b[property];
    else if((typeof a[property]) === 'string') {
      const aStr = a[property].toUpperCase();
      const bStr = b[property].toUpperCase();
...
const detectTicTacToeWin = (board) => {
  ...
  board.forEach((row, rowIdx) => {
    row.forEach((col, colIdx, arr) => {
      if(arr[rowIdx][colIdx]) {
        if(arr[rowIdx][colIdx].toLowerCase() === 'x') {
          if(xArr) {
            xArr.forEach(pair => {
              pair.x += rowIdx;
              pair.y += colIdx;
            });
            if(winCheck(xArr)) return true;
          }
          xArr.push({rowIdx, colIdx});
        }
        else if(arr[rowIdx][colIdx].toLowerCase() === 'o') {
          if(oArr) {
            oArr.forEach(pair => {
              pair.x += rowIdx;
              pair.y += colIdx;
...
const winCheck = (arr) => {
  arr.forEach(pair => {
    if(pair.x === 3 && pair.y === 3) return true;
    else if(pair.x === 3 && pair.y === 0) return true;
    else if(pair.y === 3 && pair.x === 0) return true;
  });
  return false;
};
